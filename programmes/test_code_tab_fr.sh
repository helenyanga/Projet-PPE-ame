...
#Créer un dossier tableau pour le tableau html :
mkdir -p ./tableaux
mkdir -p ./dumps-text
mkdir -p ./aspirations
...
do
    #On crée des variables pour l'HTTP, l'encodage, le nombre de mots et le fichier de sortie pour que les résultats se génèrent à l'intérieur de ce même fichier.
     fichier_data=$(curl -s -i -L -w "%{http_code}\n%{content_type}" -o ./.aspirations/fr-$N.html $line) #Récupérer la page web avec ses métadonnées et les sauvegarder dans le fichier fr-numérodelalignedelurl.html au sein du dossier français.
    ...
    echo -e "${N}\t${line}\t${http_code}\t${content_type}\t${nb_mots}\t${nb_occurenres}\t${aspirations}\t${dumps}" >> $fichier_sortie #Les chevrons permettent d'envoyer les métadonnées dans le fichier de sortie "tsv".
    N=$( expr $N + 1 )
